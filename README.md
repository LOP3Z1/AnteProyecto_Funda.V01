...
graph TD
    ...
    %% Registro con Ingreso de Monto
    Reg --> RegInput[Pedir Usuario y Clave]
    RegInput --> RegMonto{¿Desea ingresar un monto inicial?}
    RegMonto -->|Sí| PedirMonto[Pedir Monto Personalizado]
    RegMonto -->|No| SetDefault[Asignar Monto Fijo $500]
    
    PedirMonto --> RegSave[Guardar en usuarios.txt]
    SetDefault --> RegSave
    RegSave --> MenuP
    ...
    class MenuP,BankMenu,Ahorcado,Reg,RegInput,RegSave,LogInput,LogFail,Retiro,DoRetiro,ErrorSaldo,PayAhorcado,Play,WinPrize,LoseMsg,ErrorCosto,PedirMonto,SetDefault proceso;
    class OpciónP,LogCheck,OpciónB,CheckSaldo,CheckCosto,GameLoop,CheckWin,RegMonto decision;
